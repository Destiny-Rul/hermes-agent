...
- **Hide vs. Close for the in-app browser** — a zone holding a Browser (or HTML preview) offers **Hide** instead of Minimize. Hiding collapses the zone to its restore rail but keeps the live page mounted: unsaved form input, timers, scroll position, and the agent's `drive_preview` automation target all survive, and **Restore** shows the same document without a reload. The hidden page is inert — it never takes keyboard shortcuts or steals composer focus. **Close** (the tab's ×) is what actually releases the page.
